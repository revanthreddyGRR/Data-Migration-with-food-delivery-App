...
list @stage_rawdata.csv_stg/initial/delivery/;


-- this table may have additional information like picked time, accept time etc.
create or replace table stage_rawdata.delivery (
...
create or replace stream stage_rawdata.delivery_stm 
on table stage_rawdata.delivery
append_only = true
comment = 'this is the append-only stream object on delivery table that only gets delta data';


copy into stage_rawdata.delivery (deliveryid,orderid, deliveryagentid, deliverystatus, 
...
from (
    ...
    from @stage_rawdata.csv_stg/initial/delivery/delivery-initial-load.csv t
)
...
CREATE OR REPLACE TABLE clean_datacleaning.delivery (
...
create or replace stream CLEAN_datacleaning.delivery_stm 
on table CLEAN_datacleaning.delivery
...
MERGE INTO 
    clean_datacleaning.delivery AS target
USING 
    stage_rawdata.delivery_stm AS source
ON 
