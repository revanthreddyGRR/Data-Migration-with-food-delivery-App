...
create or replace table stage_rawdata.agents (
...
create or replace stream stage_rawdata.agents_stm 
on table stage_rawdata.agents
append_only = true
...
copy into stage_rawdata.agents(deliveryagentid, name, phone, vehicletype, locationid, 
...
from (
    ...
    from @stage_rawdata.csv_stg/initial/agents t
)
file_format = (format_name = 'stage_rawdata.csv_file_format')
on_error = abort_statement;


select count(*) from stage_rawdata.agents;

select * from stage_rawdata.agents_stm;



CREATE OR REPLACE TABLE clean_datacleaning.agents (
...
create or replace stream CLEAN_datacleaning.agents_stm 
on table CLEAN_datacleaning.agents
comment = 'This is the stream object on delivery agent table table to track insert, update, and delete changes';




MERGE INTO clean_datacleaning.agents AS target
USING stage_rawdata.agents_stm AS source
...
select * from CLEAN_datacleaning.agents_stm ;

CREATE OR REPLACE TABLE consumption.agents_dim (
...
MERGE INTO consumption.agents_dim AS target
USING CLEAN_datacleaning.agents_stm AS source
ON 
...
copy into agents (deliveryagentid, name, phone, vehicletype, locationid, 
                         status, gender, rating, createddate, modifieddate,
                         _stg_file_name, _stg_file_load_ts, _stg_file_md5, _copy_data_ts)
from (
    select 
        ...
        metadata$file_content_key as _stg_file_md5,
        current_timestamp as _copy_data_ts
    from @stage_rawdata.csv_stg/delta/agents/day-02-delivery-agent.csv t
)
file_format = (format_name = 'stage_rawdata.csv_file_format')
on_error = abort_statement;
