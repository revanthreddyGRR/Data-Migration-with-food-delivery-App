...
create or replace table stage_rawdata.orders (
...
create or replace stream stage_rawdata.orders_stm 
on table stage_rawdata.orders
append_only = true
comment = 'This is the append-only stream object on orders entity that only gets delta data';


list  @stage_rawdata.csv_stg/initial/orders/orders-initial.csv;


copy into stage_rawdata.orders (orderid, customerid, restaurantid, orderdate, totalamount, 
...
from (
    ...
    from @stage_rawdata.csv_stg/initial/orders/orders-initial.csv t
)
file_format = (format_name = 'stage_rawdata.csv_file_format')
on_error = abort_statement;


CREATE OR REPLACE TABLE CLEAN_datacleaning.ORDERS (
...
create or replace stream CLEAN_datacleaning.ORDERS_stm 
on table CLEAN_datacleaning.ORDERS
comment = 'This is the stream object on ORDERS table table to track insert, update, and delete changes';


MERGE INTO CLEAN_datacleaning.ORDERS AS target
USING STAGE_rawdata.ORDERS_STM AS source
...
-- part-2
list @stage_rawdata.csv_stg/delta/orders/;
copy into stage_rawdata.orders (orderid, customerid, restaurantid, orderdate, totalamount, 
                  status, paymentmethod, createddate, modifieddate,
                  _stg_file_name, _stg_file_load_ts, _stg_file_md5, _copy_data_ts)
from (
    select 
        ...
        metadata$file_content_key as _stg_file_md5,
        current_timestamp as _copy_data_ts
    from @stage_rawdata.csv_stg/delta/orders/day-02-orders.csv t
)
file_format = (format_name = 'stage_rawdata.csv_file_format')
on_error = abort_statement;
