SANDBOX.STAGE_RAWDATA.RESTAURANT_STM-- change context
use role sysadmin;
use database sandbox;
use schema stage_rawdata;
use warehouse adhoc_wh;

-- create restaurant table under stage location, with all text value + audit column for copy command
create or replace table stage_rawdata.restaurant (
    restaurantid text,      
...
create or replace stream stage_rawdata.restaurant_stm
on table stage_rawdata.restaurant
append_only = true
...
copy into stage_rawdata.restaurant (restaurantid, name, cuisinetype, pricing_for_2, restaurant_phone, 
...
from (
    select 
     ...
     from @stage_rawdata.csv_stg/initial/restaurant/restaurant-delhi+NCR.csv t
)
...
create or replace table clean_datacleaning.restaurant (
...
create or replace stream clean_datacleaning.restaurant_stm 
on table clean_datacleaning.restaurant
...
insert into clean_datacleaning.restaurant (
    restaurant_id,
    name,
    cuisine_type,
    pricing_for_two,
...
select 
    ...
    _stg_file_md5
from 
    stage_rawdata.restaurant;


    
        
-- here is the merge statement
MERGE INTO clean_datacleaning.restaurant AS target
USING (
    SELECT 
        try_cast(restaurantid AS number) AS restaurant_id,
        try_cast(name AS string) AS name,
...
CREATE OR REPLACE TABLE CONSUMPTION.RESTAURANT_DIM (
...
select count(*) from clean_datacleaning.RESTAURANT_STM;

-- merge statement
MERGE INTO 
    CONSUMPTION.RESTAURANT_DIM AS target
USING 
    clean_datacleaning.RESTAURANT_STM AS source
ON 
...
list @stage_rawdata.csv_stg/daily/restaurant/;
...
from (
    select 
        ...
        metadata$file_content_key as _stg_file_md5,
        current_timestamp() as _copy_data_ts
     from @stage_rawdata.csv_stg/daily/restaurant/day-02-upsert-restaurant-delhi+NCR.csv t
)
file_format = (format_name = 'stage_rawdata.csv_file_format')
on_error = abort_statement;
...
list @stage_rawdata.csv_stg/delta/restaurant/;
SANDBOX.STAGE_RAWDATA.RESTAURANTSANDBOX.STAGE_RAWDATA.RESTAURANT
