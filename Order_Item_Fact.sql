...
CREATE OR REPLACE TABLE consumption.order_item_fact (
...
MERGE INTO consumption.order_item_fact AS target
USING (
    SELECT 
    ...
    FROM 
        CLEAN_datacleaning.items_stm oi
    LEFT JOIN 
        CLEAN_datacleaning.ORDERS_stm o ON oi.Order_ID_fk = o.Order_ID
    LEFT JOIN 
        clean_datacleaning.delivery_stm d ON o.Order_ID = d.Order_ID_fk
    LEFT JOIN 
        consumption.CUSTOMER_DIM c on o.Customer_ID_fk = c.customer_id
    LEFT JOIN 
        consumption.CUSTOMER_ADDRESS_DIM ca on c.Customer_ID = ca.CUSTOMER_ID_fk
    LEFT JOIN 
        consumption.restaurant_dim r on o.Restaurant_ID_fk = r.restaurant_id
    LEFT JOIN 
        consumption.menu_dim m ON oi.MENU_ID_fk = m.menu_id
    LEFT JOIN 
        consumption.agents_dim da ON d.Delivery_Agent_ID_fk = da.delivery_agent_id
    LEFT JOIN 
        consumption.restaurant_location_dim rl on r.LOCATION_ID_FK = rl.location_id
    LEFT JOIN 
        CONSUMPTION.DATE_DIM dd on dd.calendar_date = date(o.order_date)
) AS source_stm
ON 
