...
create or replace table stage_rawdata.menu (
...
create or replace stream stage_rawdata.menu_stm 
on table stage_rawdata.menu
append_only = true
comment = 'This is the append-only stream object on menu entity that only gets delta data';


list @stage_rawdata.csv_stg/initial/menu;



copy into stage_rawdata.menu (menuid, restaurantid, itemname, description, price, category,availability, itemtype, createddate, modifieddate,_stg_file_name, _stg_file_load_ts, _stg_file_md5, _copy_data_ts)
from (
    select 
        t.$1::text as menuid,
    ...
    from @stage_rawdata.csv_stg/initial/menu t
)
...
select *from table(information_schema.copy_history(table_name=>'MENU', start_time=> dateadd(hours, -1, current_timestamp())));



CREATE OR REPLACE TABLE clean_datacleaning.menu (
...
comment = 'Menu entity under clean schema with appropriate data type under clean schema layer, data is populated using merge statement from the stage layer location table. This table does not support SCD2';
    ...
    ALTER TABLE CLEAN_DATACLEANING.MENU 
  ALTER COLUMN PRICE 
    DROP NOT NULL;

create or replace stream CLEAN_datacleaning.menu_stm 
on table CLEAN_datacleaning.menu
comment = 'This is the stream object on menu table table to track insert, update, and delete changes';



MERGE INTO clean_datacleaning.menu AS target
USING (
    SELECT 
...
CREATE OR REPLACE TABLE consumption.menu_dim (
...
MERGE INTO 
    consumption.MENU_DIM AS target
USING 
    CLEAN_datacleaning.MENU_STM AS source
ON 
...
-- Part -2
list @stage_rawdata.csv_stg/delta/menu;
copy into stage_rawdata.menu (menuid, restaurantid, itemname, description, price, category, 
...
from (
    select 
        ...
        metadata$file_content_key as _stg_file_md5,
        current_timestamp as _copy_data_ts
    from @stage_rawdata.csv_stg/delta/menu/ t
)
file_format = (format_name = 'stage_rawdata.csv_file_format')
on_error = abort_statement;
