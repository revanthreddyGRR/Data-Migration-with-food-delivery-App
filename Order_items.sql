...
create or replace table stage_rawdata.items (
...
create or replace stream stage_rawdata.items_stm 
on table stage_rawdata.items
append_only = true
comment = 'This is the append-only stream object on order item table that only gets delta data';

list @stage_rawdata.csv_stg/initial/items;




copy into stage_rawdata.items (orderitemid, orderid, menuid, quantity, price, 
...
from (
    ...
    from @stage_rawdata.csv_stg/initial/items/ t
)
file_format = (format_name = 'stage_rawdata.csv_file_format')
on_error = abort_statement;


select * from stage_rawdata.items;
select * from stage_rawdata.items_stm;



CREATE OR REPLACE TABLE clean_datacleaning.items (
...
create or replace stream CLEAN_datacleaning.items_stm 
on table CLEAN_datacleaning.items
comment = 'This is the stream object on order_item table table to track insert, update, and delete changes';

select * from clean_datacleaning.items_stm;


MERGE INTO clean_datacleaning.items AS target
USING stage_rawdata.items_stm AS source
ON  
...
WHEN NOT MATCHED THEN
    ...
    list @stage_rawdata.csv_stg/delta/items/;
...
from (
    select 
        ...
        metadata$file_content_key as _stg_file_md5,
        current_timestamp as _copy_data_ts
    from @stage_rawdata.csv_stg/delta/items/ t
)
file_format = (format_name = 'stage_rawdata.csv_file_format')
on_error = abort_statement;
